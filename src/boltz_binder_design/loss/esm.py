...
def boltz_to_esm_matrix():
    ...
    T = np.zeros((20, len(ESM_TOKENS)))
    for i, tok in enumerate([tokens[i] for i in range(2, 22)]):
        one_letter_code = prot_token_to_letter[tok]
        esm_idx = ESM_TOKENS.index(one_letter_code)
        T[i, esm_idx] = 1
    return T
