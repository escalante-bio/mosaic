...
# we should really, really stop hardcoding this everywhere.
TOKENS = tokens[2:22]
...
def load_trigram_frequencies(path: Path = "../../../trigram_seg.pkl"):
    ...
    for trimer, freq in tqdm(ngram_dict.items()):
        three_letter_codes = list(trimer)
        if all(
            c in prot_letter_to_token and prot_letter_to_token[c] in TOKENS
            for c in three_letter_codes
        ):
            i, j, k = [TOKENS.index(prot_letter_to_token[c]) for c in trimer]
            P[i, j, k] = freq
