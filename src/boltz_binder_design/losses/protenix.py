...
#         return (
#             jax.nn.softmax(self.confidence_metrics.pae_logits[0])
...
#     @property
#     def iptm(self):
#         asym_id = self.features["asym_id"]

#         return predicted_tm_score(
#             asym_id=asym_id,
#             logits=self.pae_logits,
#             breaks=np.arange(start=0.5 * 0.5, stop=32.0, step=0.5)[:-1],
#             interface=True,
#         )
...
class ProtenixOutput(AbstractStructureOutput):
    ...
    def iptm(self):
        ...
        )(self.confidence_metrics.pae_logits)
