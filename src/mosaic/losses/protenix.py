...
from mosaic.common import TOKENS, LinearCombination, LossTerm, StateIndex
...
#         # TODO: This shouldn't live here - it should be in protenij.
...
#                 z = z + self.model.template_embedder(
#                     self.features, z, pair_mask=None, key=key
#                 )
...
#                 key=key,
#             )
#             s = initial_embedding.s_init + self.model.linear_no_bias_s(
#                 self.model.layernorm_s(s)
#             )
...
#             lambda carry: carry[0] < self.recycling_steps -1,
#             body_fn,
#             (0, state, self.key),
#         )
#         state = jax.tree.map(jax.lax.stop_gradient, state)
#         return body_fn((0, state, key))[1]
